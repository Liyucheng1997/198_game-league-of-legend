...
默认进入走位挑战：选择操控英雄后，点击「开始走位挑战」。右键点击或按住移动，S 停止，D 闪现，Esc 暂停。90 秒内躲避技能得分，累计命中 10 次结束。

- 十位对手轮换，也可锁定单个对手练习；入门、标准、进阶三个难度。
- 躲避 +100，连击逐次 +10（最高额外 +200），弹道擦边额外 +50。每次施法仅结算一次。
- 使用所选英雄的基础移速；闪现 400 距离、15 秒训练冷却。右键实时改向，没有人为转向延迟。
- 可关闭辅助预警；窄窗口点「设置」切换训练选项。设置通过「应用设置 / 重新开始」生效。
- 结算显示躲避率、最高连击和本机最高分。本机最高分合并各难度，仅供个人练习。
- [模型校准页](models.html) 展示十个经典模型，支持待机、跑步、Q 动画和四向转身。

技能形状参考原英雄机制；参数集中在 `js/dodge-core.js`，属于可调整的训练近似，未声称与某个正式补丁 1:1。易训练保持距离（Q 点名不可走位躲避），盖伦训练脱离审判范围。训练模式不模拟完整控制链、装备、伤害或网络延迟。原 5V5 和自由训练入口继续可用。

运行 `node tests/dodge.test.js` 验证走位机制，`node tests/rift.test.js` 验证原模式。详细参数说明见 [走位模式说明](docs/dodge-mode.md)。
...
[在线试玩](https://liyucheng1997.github.io/198_game-league-of-legend/) · [v1.3 发布说明](https://github.com/Liyucheng1997/198_game-league-of-legend/releases/tag/v1.3)
...
若要使用原自由训练场，先选英雄，再点「训练模式」和「进入训练模式」。训练场提供满级、金币、敌我训练目标、冷却重置、全图视野和兵线生成。
